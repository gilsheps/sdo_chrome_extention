...
function onPageLoad(Page) {
  ...
  switch (Page) {
    case "Home":
      try {
        let isAlertInfoOpen =
          document.querySelector("#Home #apps").style.display == "block";
        if (isAlertInfoOpen) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  var getSelectedHost = await Preferences.getSelectedHost();
  if (getSelectedHost) {
    console.log("getSelectedHost", getSelectedHost);
    currentPage = "Home";
    document.querySelector('.tablink[forPageID="' + currentPage + '"]').click();
    ...
    console.log("getSelectedHost nooooooooo");
    currentPage = "Config";
    document.querySelector('.tablink[forPageID="' + currentPage + '"]').click();
  }
});

document.getElementById("url").onload = async function (element) {
  var host = await Preferences.getSelectedHost();
  console.log("url from storage", host, element);
  if (host) {
...
document.getElementById("update").onclick = async function (e) {
  ...
  await Preferences.saveSelectedHost(urlText);
  //
  currentPage = "Home";
  document.querySelector('.tablink[forPageID="' + currentPage + '"]').click();
};
