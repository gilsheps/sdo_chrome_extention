...
function onLoad() {
  chrome.cookies.get({ url: `${portalUrl}`, name: "token" }, function (cookie) {
    if (!cookie) {
      console.error("Cant get cookie! Check the name!");
    }
  });
  let services = [];
  $.ajax({
    type: "GET",
    url: `${portalUrl}/portal/api/services`,
    crossDomain: true,
    dataType: "json",
    xhrFields: {
...
function addApp(appsDiv, app) {
  ...
  if (app == sdo) {
    ...
    div.onclick = function () {
      launchApp(`${portalUrl}${app.loginUrl}`);
    };
  }
